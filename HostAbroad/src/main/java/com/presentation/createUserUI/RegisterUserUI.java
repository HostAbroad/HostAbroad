...
public class RegisterUserUI extends UI{
	...
	protected void init(VaadinRequest request) {
		...
		binder.forField(fullName).withValidator(new RegexpValidator("Field full name should have a least 4 letters.", "^.{3,60}\\D+$"))
		.bind("fullName");
		
		binder.forField(nickname).withValidator(new StringLengthValidator("Field nickname should contains between 4 and 20 characters.", 4, 20))
		.bind("nickname");
		
		binder.forField(email).withValidator(new EmailValidator("Invalid e-mail address {0}."))
		.bind("email");
		binder.forField(password).withValidator(new RegexpValidator("Should contains minimum eight characters, at least one uppercase letter, one lowercase letter and one number.", "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[a-zA-Z\\d]{8,}$")) 
		.bind("password");
		...
		Panel panel = createPanel(registerLayout);
	...
	private Image loadImage(String url) { //This method load all images. We should put it in a separate class in the future.
        //reading the image
        //-----------------------------------
